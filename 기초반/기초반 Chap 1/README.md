...
## [1](#toc). Python 개발환경 설치

- 해당 강의 자료의 개발 환경은 Python 3.13, VS Code, Windows 11
- 자신이 차후 사용하고자 하는 목적에 따라 환경 설정이 달라질 수 있다.
  - 3.9 이상부터는 Windows 7 이상에서만 지원된다.
  - 일부 라이브러리의 버전은 이전 버전에서만 호환될 수 있으며 Linux를 필요로 할 수 있다.
...
#### [1-1-1](#toc). Python 3.10 vs 3.11 vs 3.12

- 매년 10월 즈음에 차기 버전이 발표되며 25년 10월에 3.14가 공개될 예정이다.
...
## [3](#toc). Python 개발을 위해 도움이 될만한 것들

### LLM 서비스(OpenAI, Claude, Gemini, Qwen, Grok 등)

- 그는 신이다. 이 강의자료가 처음 쓰일 당시에는 Chat GPT가 존재하지 않았고 23년도 11월 말-12월 초에 처음 서비스가 탄생했다.
- Github에서는 Copilot X라는 코딩 에이전트를 통해 LLM을 무료로 제공해준다. (학생 계정 인증 필요)
